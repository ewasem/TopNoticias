...
object MockData {
    ...
    //Notícias para preparar a UI para receber os dados que futuramente virão da API de notícias.
    val topNewsList = listOf<NewsData>(
        NewsData(
            ...
            publishedAt = "2021-12-06T10:35:21Z"
        ),
        NewsData(
            2,
            R.drawable.namita,
    ...
    fun Date.getTimeAgo(): String {
        ...
        return if (year<currentYear){
            ...
            if(interval == 1) "$interval ano atrás" else "$interval anos atrás"
        } else if (month < currentMonth) {
            val interval = currentMonth - month
            if (interval == 1) "$interval mês atrás" else "$interval meses atrás"
        } else if (day < currentDay) {
            val interval = currentDay - day
            if (interval == 1) "$interval dia atrás" else "$interval dias atrás"
        } else if (hour < currentHour) {
            val interval = currentHour - hour
            if (interval == 1) "$interval hora atrás" else "$interval horas atrás"
        } else if (minute < currentMinute){
            val interval = currentMinute - minute
            if (interval == 1) "$interval minuto atrás" else "$interval minutos atrás"
        } else {
            "Agora mesmo"
        }
    }

    //Transforma a String da data recebida no formato de Data
    fun stringToDate(publishedAt: String): Date {
        val date =
            //Se a API do telefone for acima ou igual a 24, utiliza o SimpleDateFormat atual, senão utiliza o antigo
